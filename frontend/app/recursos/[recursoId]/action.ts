...
export async function eliminarCargaHoras(cargaHorasId: string) {
  try {
    const res = await fetch(
      `${process.env.BACKEND_URL}/carga-de-horas/${cargaHorasId}`,
      {
        method: "DELETE",
        headers: {
    ...
    console.info(
      `Carga de horas con ID ${cargaHorasId} eliminada correctamente.`
    );
    ...
    return {
      ...
      mensaje:
        ...
        `Error interno al intentar eliminar la carga de horas con ID ${cargaHorasId}.`
    };
  }
}
